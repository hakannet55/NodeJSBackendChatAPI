...
async function createTable() {
    try {
        ...
        if (exist.rowCount == 0) {
            ...
            const insertUserRootSql = `INSERT INTO ${tabNameEnums.tbl_prod_users} (account, username, password)
                                       VALUES ('nelly', 'root', '${hashedPassword}');`;
...
const setData = (tabNameEnums, data) => {
    if (typeof data == 'object') {
        return new Promise((resolve, reject) => {
            ...
            const sqlInsert = `INSERT INTO ${tabNameEnums} (${columns})
                               VALUES (${values})`;
